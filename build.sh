...
# Strip editor hint comments from the top of a file
strip_editor_hints() {
  local input_file="$1"
  local skip_editor_hints=1
  local in_local_vars=0
  local found_editor_hint=0
  
  while IFS= read -r line || [[ -n "$line" ]]; do
    # Check if we're still in the editor hints section
    if [[ $skip_editor_hints -eq 1 ]]; then
      # Skip lines starting with "# Editor hints:"
      if echo "$line" | grep -qE '^[[:space:]]*# Editor hints:'; then
        found_editor_hint=1
        continue
      fi
      
      # Skip vim modeline
      if echo "$line" | grep -qE '^[[:space:]]*# vim:'; then
        found_editor_hint=1
        continue
      fi
      
      # Track Local Variables block
      if echo "$line" | grep -qE '^[[:space:]]*# Local Variables:'; then
        in_local_vars=1
        found_editor_hint=1
        continue
      fi
      
      # Skip lines within Local Variables block
      if [[ $in_local_vars -eq 1 ]]; then
        ...
        # Skip all lines in Local Variables block
        continue
      fi
      
      # If we've found editor hints and hit a non-editor-hint line, stop skipping
      # If we haven't found any editor hints yet, this might be before them, so output it
      if [[ $found_editor_hint -eq 1 ]]; then
        skip_editor_hints=0
      fi
    fi
...
if [[ "$MINIFY" == "true" ]]; then
  ...
  CLEANUP_FILES+=("$STRIPPED_FILE")
  strip_editor_hints "$SRC_DIR/common.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying adapter_registry_helpers.sh...${NC}"
  strip_editor_hints "$SRC_DIR/adapter_registry_helpers.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying adapter_registry.sh...${NC}"
  strip_editor_hints "$SRC_DIR/adapter_registry.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying framework_detector.sh...${NC}"
  strip_editor_hints "$SRC_DIR/framework_detector.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying adapters/bats.sh...${NC}"
  strip_editor_hints "$SRC_DIR/adapters/bats.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying adapters/rust.sh...${NC}"
  strip_editor_hints "$SRC_DIR/adapters/rust.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying scanner.sh...${NC}"
  strip_editor_hints "$SRC_DIR/scanner.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager_docker.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager_docker.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager_core_helpers.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager_core_helpers.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager_build_helpers.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager_build_helpers.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager_container.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager_container.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager_execution.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager_execution.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager_integration.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager_integration.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying build_manager.sh...${NC}"
  strip_editor_hints "$SRC_DIR/build_manager.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"

  echo -e "${YELLOW}  Adding and minifying main.sh...${NC}"
  strip_editor_hints "$SRC_DIR/main.sh" > "$STRIPPED_FILE"
  minify_script "$STRIPPED_FILE" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/adapter_registry.sh" >> "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/framework_detector.sh" >> "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/adapters/bats.sh" >> "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/adapters/rust.sh" >> "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/scanner.sh" >> "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/build_manager.sh" >> "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  ...
  echo "# ============================================================================" >> "$TEMP_FILE"
  strip_editor_hints "$SRC_DIR/main.sh" >> "$TEMP_FILE"
fi
